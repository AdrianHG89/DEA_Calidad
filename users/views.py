"""
Este fichero views.py ha sido creado con
la finalidad de alojar las vistas de las
distintas funcionalidades de nuestra aplicacion
relacionadas con nuestro modelo de usuario.
"""
...
from post.models import Post, validate_email
...
def login_user(request):
    """
	Es invocada cuando hacemos un login y se encarga de recoger los datos y validarlos.
    """
...
def logout_view(request):
    """
    Es invocada cuando hacemos un cierre de sesion.
    """
    logout(request)
...
def update(request):
    """
    Es invocada cuando hacemos algun tipo de modificacion en la informacion personal del usuario.
    """
...
def follow(request, user_id):
    """
    Es invocada cuando pulsamos el boton seguir para indicar que queremos seguir un usuario.
    """
...
def unfollow(request, user_id):
    """
    Es invocada cuando pulsamos el boton dejar para indicar que queremos dejar de seguir un usuario.
    """
    if not request.user.is_authenticated():
        return redirect('/login/?next=%s' % request.path)
    else:
...
def deluser(request):
    """
    Es invocada cuando decidimos borrar una cuenta de usuario.
    """
    if not request.user.is_authenticated():
        return redirect('/login/?next=%s' % request.path)
    else:
...
def perfil(request, user_id):
    """
    Es invocada cuando un usuario quiere visitar se pefil
    """
    if not request.user.is_authenticated():
        return redirect('/login/?next=%s' % request.path)
    else:
